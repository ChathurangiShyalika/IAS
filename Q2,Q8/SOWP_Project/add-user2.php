...
$addOrEdit="Add New";
		...
		if(isset($_GET["eid"])){
			$eid ='<input type="hidden" name="eid" value="'.$_GET["eid"].'"/>';
			$addOrEdit="Edit";
			//$id= ($_GET["eid"]);
			$getInstrs = mysqli_fetch_row(mysqli_query($conn, 'SELECT * FROM user WHERE User_ID='.$_GET["eid"].''));
}



$lnameErr = $fnameErr = $addressErr = $emailErr = $catErr = $cntErr = $dateErr = $dayErr = $subjErr = $rateErr = $utypeErr = $pwErr =$genderErr= $dobErr= $id="";
...
if (empty($_POST["Email"])) {
 ...
 if (empty($_POST["DOB"])) {
  ...
  if (empty($_POST["subjects"])) {
    $subjErr = "Subject/s are required";
	}else{
   ...
   if (empty($_POST["days"])) {
    $dayErr = "Available day /s are required";
	}else{
...
mysqli_query($conn,'UPDATE user SET User_Name="'.$_POST['Email'].'",Password="'.$_POST['Password'].'",User_Type="'.$_POST['utype'].'",First_Name="'.$_POST['FirstName'].'",Last_Name="'.$_POST['LastName'].'",Address="'.$_POST['Address'].'",Gender="'.$_POST['Gender'].'",Date_Of_Birth="'.$_POST['DOB'].'",TP_No="'.$_POST['ContactNumber'].'" WHERE user.User_ID = '.$id);
...
mysqli_query($conn,'UPDATE user SET User_Name=$email,Password=$password,User_Type=$utype,First_Name=$first_name,Last_Name=$last_name,Address=$address,Gender=$gender,Date_Of_Birth=$dob,TP_No=$contact_no WHERE User_ID = '.$id) or die(mysqli_error($conn));
